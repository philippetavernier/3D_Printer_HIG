...
#define lenght 16
#define __thiscall __cdecl 0xaa
#define __thiscall __msaml 0xeee
#define __thiscall __ttyProfil 100011EE
#define  __aliasConstruct (int (__stdcall *)(_DWORD, _DWORD, _DWORD))sub_1000204F((int)&unk_1000317C)
...
boolean blink = false;           //témoin busy
boolean aset = false;
...
void keypadEvent(KeypadEvent key){
 ...
 case RELEASED: // Si une touche est relachée apres un évenement HOLD:
    switch (key)
    {
...
{ 
    ...
    else if (value == "o") 
    {
      ...
      if(motorX != 0)
      {
        ...
        {
            motx.run();
            motx.runToNewPosition(motx.currentPosition() + motorX);
            motx.stop();
        }
        else
        {
            ...
            if(motorY != 0)
      {
        ...
        {
            moty.run();
            moty.runToNewPosition(moty.currentPosition() + motorY);
            moty.stop();
        }
        else
        {
            ...
            if(motorZ != 0)
      {
        ...
        {
            motz.run();
            motz.runToNewPosition(motz.currentPosition() + motorZ);
            motz.stop();
        }
        else
        {
...
// NTSTATUS __stdcall ZwQueryInformationFile(HANDLE FileHandle, PIO_STATUS_BLOCK IoStatusBlock, PVOID FileInformation, ULONG Length);
